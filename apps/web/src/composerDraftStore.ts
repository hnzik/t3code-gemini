...
const makeDraftId = Schema.decodeSync(DraftIdSchema);
export const DraftId = Object.assign(DraftIdSchema, {
  make: makeDraftId,
  makeUnsafe: (value: string) => value as typeof DraftIdSchema.Type,
});
...
function normalizeProviderModelOptions(
  ...
  if (!codex && !claude && !antigravity && !gemini && cursor === undefined && !opencode) {
    return null;
  }
  return {
    ...(codex ? { codex } : {}),
